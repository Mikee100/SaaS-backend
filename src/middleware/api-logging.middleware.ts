...
export class ApiLoggingMiddleware implements NestMiddleware {
  ...
  async use(req: Request, res: Response, next: NextFunction) {
    ...
    // Log the API request
    try {
      await this.auditLogService.log(userId, 'api_request', {
        method: req.method,
        path: req.path,
        query: req.query,
    ...
    res.on('finish', async () => {
      ...
      try {
        await this.auditLogService.log(userId, 'api_response', {
          method: req.method,
          path: req.path,
  ...
  private sanitizeBody(body: any): any {
    ...
    const sensitiveFields = ['password', 'token', 'secret', 'key', 'apiKey', 'stripeToken'];
