...
export class SectionLogoService {
  ...
  private parseLogoSettings(settings: any): SectionLogoSettings {
    ...
    for (const [section, config] of Object.entries(result.sections)) {
      result.sections[section] = {
        ...defaultSettings.sections[section as keyof typeof defaultSettings.sections],
        ...config
      };
    }
  ...
  async getSectionLogoConfig(tenantId: string, section: string): Promise<SectionLogoConfig | null> {
  ...
  async updateSectionLogoConfig(
    ...
    return this.getSectionLogoConfig(tenantId, section) as Promise<SectionLogoConfig>;
  }
  ...
  async getAllSectionLogos(tenantId: string): Promise<Record<string, SectionLogoConfig>> {
    const tenant = await this.prisma.tenant.findUnique({
      where: { id: tenantId },
      select: {
        logoSettings: true,
  ...
  async updateSectionLogo(
    ...
    const currentSettings: SectionLogoSettings = this.parseLogoSettings(tenant.logoSettings);
  ...
  async removeSectionLogo(tenantId: string, sectionName: string): Promise<boolean> {
    const tenant = await this.prisma.tenant.findUnique({
      where: { id: tenantId },
      select: { logoSettings: true },
    });
  ...
  private getDefaultLogoForSection(tenant: any, section: string): SectionLogoConfig {
  ...
  async getLogoUsageBySection(tenantId: string): Promise<Record<string, string | null>> {
    const tenant = await this.prisma.tenant.findUnique({
      where: { id: tenantId },
      select: {
        logoUrl: true,
